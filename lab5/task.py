...
def ringz(t: float, x_2: list, y_2: list, n_2: int, r_2: list, x_0: list, y_0: list, a_x: list, a_y: list, color: list):
    for i in range(n_2):
        circle(screen, color[i], (round(x_2[i]), round(y_2[i])), r_2[i])
        circle(screen, BLACK, (round(x_2[i]), round(y_2[i])), r_2[i]-5)
        x_2[i] = x_0[i] + a_x[i] * m.cos(0.7 * t)
        y_2[i] = y_0[i] + a_y[i] * m.sin(1.4 * t)
...
while not finished:
    ...
    ringz(t, x_2, y_2, n_2, r_2, x_0, y_0, a_x, a_y, color_2)
    t += dt
